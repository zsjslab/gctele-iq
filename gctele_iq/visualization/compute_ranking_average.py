...
def main(project_dir='.', dataset_name=None, list_models=None):
    ...
    dataset_name = [ds.strip() for ds in dataset_name if ds and ds.strip()]  # 去除空字符串
    ...
    for ds_name in dataset_name:
        for model in MODELS:
            try:
                entries = load_scores(project_dir, ds_name, model)
                for entry in entries:
                    entry['dataset'] = ds_name
                combined_data[model].extend(entries)
            except FileNotFoundError as e:
                print(f"[警告] 跳过模型 {model} 在数据集 {ds_name}，原因：{e}")
...
if __name__ == '__main__':
    ...
    parser.add_argument('--dataset_name', nargs='+', required=True, help='空格分隔的多个数据集名称, 如A B C')
    parser.add_argument('--list_models', type=str, help='逗号分隔的模型名称列表, 如 qwen2.5-7b-instruct,deepseek-v3')
...
# python -m gctele_iq.visualization.compute_ranking_average --dataset_name A B C
